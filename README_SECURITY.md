...
│     • gRPC rpcx：s = SHA256(规范字段)，e = Ed25519(s)，P=0 明文 d │
│     • 响应侧同样 Ed25519 验签（rpcx 响应含 c、m 进摘要）          │
│                                          │
│  ✅ 动态密钥生成                         │
│     • PBKDF2 密钥派生                    │
│     • 10,000 次迭代                      │
...
│     • gRPC rpcx：SHA256 规范串 + Ed25519(s)，无对称 MAC           │
│                                          │
│  ✅ 防篡改机制                           │
│     • 多路径：HMAC/GCM/Ed25519 或 hash-then-sign（rpcx）          │
│     • 双向（请求+响应）                   │
│     • 无法伪造有效签名（无私钥）          │
└─────────────────────────────────────────┘
```
...
### gRPC RPCX（`rpcx` / `utils/sdk`）

与 HTTP/WebSocket 的「HMAC + 外层 Ed25519」并行一套 **仅 gRPC** 的约定（`CommonWorker.Do`）：
...
| **篡改攻击**   | 🔴 高    | HMAC/GCM + Ed25519；rpcx 为 SHA256+Ed25519(s) | 签名层 + 加密层    | ✅ 完全防护 |
...
│     • 六层防护体系                                         │
│     • 15 种攻击防护                                        │
│     • 双向 Ed25519（HTTP/WS 外层；gRPC rpcx 对摘要 s）        │
│                                                            │
│  🔑 密钥管理          ████████████████████  100/100       │
│     • 动态生成 + PBKDF2                                    │
│     • 不存储敏感信息                                       │
...
- ✅ 双向 Ed25519：HTTP/WS 外层签摘要；gRPC rpcx 签 SHA256(s)
- ✅ JWT 认证 + RBAC 授权
- ✅ 动态密钥生成 (PBKDF2)
- ✅ AAD 上下文绑定 (Time + Nonce + Plan + Path)
- ✅ 三级限流机制
- ✅ 防重放攻击 (时间戳 + Nonce 去重)
- ✅ X25519 + ecc.EncryptX25519（Plan2 匿名通道；与 rpcx 默认 Cipher 独立）
- ✅ 零反射高性能 ORM
...
**文档版本**: v1.3 (Ed25519 / X25519 分工与 gRPC rpcx 明文摘要方案说明)
**最后更新**: 2026-04-05
**安全等级**: 🏆 金融机构级 (Tier 4)
