...
│                                          │
│  ✅ 双向 Ed25519 签名验证                    │
│     • HTTP/WebSocket：外层对 SHA256(规范串) 摘要做 Ed25519，对端公钥验签   │
│     • 可选链路 gRPC/rpcx：s = SHA256(规范字段)，e = Ed25519.Sign(SHA256(规范字段))，P=0 明文 d │
│     • 响应侧同样 Ed25519 验签（gRPC/rpcx 响应含 c、m 进摘要）               │
│                                          │
│  ✅ 动态密钥派生（Token Secret）          │
│     • HKDF-SHA256（RFC 5869，视本地动态盐启用）│
│     • HMAC-SHA256 绑定 Token 与派生材料   │
│     • 动态计算、不落库                   │
│                                          │
│  ✅ RBAC 权限控制                        │
│     • 角色匹配验证                       │
...
│     • 可选链路 gRPC/rpcx：SHA256 规范串 + Ed25519 摘要签名，无对称 MAC  │
│                                          │
│  ✅ 防篡改机制                           │
...
客户端                                服务端
  ...
  │                                    │ 11. 业务处理
  │                                    │     ↓
  │                                    │ 12. 响应 HMAC + Ed25519 摘要签名
  │◀────────  返回响应  ─────────────────┤
  │    {c, m, d, t, n, p:0, s, e}      │
  │                                    │
13. 验证 Ed25519 + HMAC 签名 & 解码     │
  │                                    │
14. 业务数据处理                       │
```
...
客户端                                服务端
  ...
  │                                    │ 13. 业务处理
  │                                    │     ↓
  │                                    │ 14. 响应 AES-GCM + HMAC + Ed25519 摘要签名
  │◀────────  返回响应  ─────────────────┤
  │    {c, m, d: encrypted,            │
  │     t, n, p:1, s, e}               │
  │                                    │
15. 验证 Ed25519 + HMAC 签名 & GCM 解密 │
  │                                    │
16. 业务数据处理                       │
```
...
客户端                                服务端
  ...
  │    {key, noc, exp, sig}            │
  │                                    │
  │  2. 验证服务端 Ed25519 摘要签名       │
  │     使用客户端预设的服务端 Ed25519 公钥 │
  ...
  │     HMAC(path+d+n+t+p+u, 派生密钥)
  │     ↓                              │
  │  9. 创建客户端 Ed25519 摘要签名 (性能优化) │
  │     使用 clientPrk 对 SHA256(path+d+n+t+p+u) 摘要签名 │
  │     对固定长度摘要签名，避免对大数据包直接签名 │
  │     → requestEd25519Sig              │
  │     ↓                              │
  ├─────────  发送请求  ────────────────▶│
  │    Header:                          │
  ...
  │                                    │
  │ 10. 验证客户端 Ed25519 摘要签名       │
  │     使用服务端预设的客户端 Ed25519 公钥 │
  │     Verify(requestEd25519Sig, SHA256(path+d+n+t+p+u))
  │     ↓                              │
  │ 11. 验证HMAC签名                    │
  │     ↓                              │
  │ 12. 检查时间戳 & Nonce             │
  ...
  │     HMAC(path+d+n+t+p+u, 派生密钥)
  │     ↓                              │
  │ 19. 响应 Ed25519 摘要签名 (双向保护)   │
  │     使用 serverPrk 对响应 SHA256(path+d+n+t+p+u) 摘要签名 │
  │     → responseEd25519Sig             │
  │◀────────  返回响应  ─────────────────┤
  │    {c, m, d: encrypted,            │
  │     t, n, p:2, s, e: responseEd25519Sig}
  │                                    │
20. 验证响应 Ed25519 摘要签名         │
  │     使用客户端预设的服务端 Ed25519 公钥 │
  │     Verify(responseEd25519Sig, SHA256(path+d+n+t+p+u))    │
  │     ↓                              │
21. 验证响应HMAC签名 & GCM解密       │
  │                                    │
 22. 业务数据处理                       │
...
1. 用户登录
   ...
   │   └─▶ HKDF-SHA256(TokenKey 材料 + 本地动态盐, info) → 中间密钥
   │       └─▶ HMAC-SHA256(类型前缀 + token, 中间密钥) → Secret
   │       • 绑定到 Token
   │       • 每次按需计算、不长期存储
   │
   └─▶ 返回客户端
...
| **中间人攻击** | 🔴 高    | TLS/HTTPS（部署）+ AES-256-GCM + 双向 Ed25519 签名                             | 传输层 + 加密层    | ✅ 有效防护 |
| **身份伪造**   | 🔴 高    | 双向 Ed25519 签名 + 密钥协商                                                   | 认证层 + 加密层    | ✅ 有效防护 |
| **篡改攻击**   | 🔴 高    | HMAC/GCM + Ed25519；可选链路 gRPC/rpcx 为 SHA256 规范串摘要 + Ed25519 摘要签名 | 签名层 + 加密层    | ✅ 有效防护 |
...
| **会话劫持**   | 🟠 中    | 动态 Secret + Token 绑定                                                       | JWT + HKDF/HMAC    | ✅ 有效防护 |
| **权限提升**   | 🟠 中    | RBAC + 角色验证                                                                | RoleFilter         | ✅ 有效防护 |
| **密钥泄露**   | 🔴 高    | 动态生成 + 不存储 + 短生命周期                                                 | GetTokenSecret     | ✅ 有效防护 |
| **Nonce 碰撞** | 🟡 低    | 12 字节随机 (2^96 空间)                                                        | crypto/rand        | ✅ 有效防护 |
...
│     • 六层防护体系                                         │
│     • 15 种攻击防护                                        │
│     • 双向 Ed25519（HTTP/WS 外层签 SHA256 规范串摘要；可选链路 gRPC/rpcx 同理） │
│                                                            │
│  🔑 密钥管理          ████████████████████  100/100       │
│     • 动态生成 + HKDF/HMAC（Token Secret）                  │
│     • 不存储敏感信息                                       │
│     • Token 绑定                                          │
│                                                            │
│  👤 认证授权          ████████████████████  100/100       │
...
- ✅ 双向 Ed25519：HTTP/WS 外层签 SHA256(规范串) 摘要；可选链路 gRPC/rpcx 同理
- ✅ JWT 认证 + RBAC 授权
- ✅ 动态密钥派生：HKDF-SHA256 + HMAC-SHA256（Token Secret）
