...
│     • 服务端使用公钥验证Ed25519签名        │
│     • 响应也包含Ed25519签名验证            │
│     • 性能优化：仅对 HMAC 摘要做 Ed25519（不对整包业务数据签名） │
│                                          │
│  ✅ 动态密钥生成                         │
│     • PBKDF2 密钥派生                    │
│     • 10,000 次迭代                      │
...
│     • HMAC-SHA256: 数据完整性             │
│     • Ed25519: 身份认证与不可否认性         │
│     • 性能优化: 外层 Ed25519 仅签 HMAC 输出（32 字节） │
│                                          │
│  ✅ 防篡改机制                           │
│     • 三重验证 (HMAC + GCM Tag + Ed25519) │
│     • 双向签名 (请求+响应)               │
│     • 无法伪造有效签名                   │
└─────────────────────────────────────────┘
```
...
| **篡改攻击**   | 🔴 高    | HMAC-SHA256 + GCM AuthTag + Ed25519 签名    | 签名层 + 加密层    | ✅ 完全防护 |
...
- ✅ AES-256-GCM 认证加密
- ✅ HMAC-SHA256 完整性验证
- ✅ 双向 Ed25519 签名验证 (性能优化：只对 HMAC 签名签名)
- ✅ JWT 认证 + RBAC 授权
- ✅ 动态密钥生成 (PBKDF2)
...
**文档版本**: v1.2 (双向 Ed25519 签名验证 + 性能优化)
**最后更新**: 2025-11-16
**安全等级**: 🏆 金融机构级 (Tier 4)
