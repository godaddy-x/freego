...
FreeGo 面向高併發與強安全場景：**Plan2 登入/金鑰交換**採用 **NIST 抗量子標準演算法（ML-KEM-1024 / ML-DSA-87）**；登入後會話與報文統一 **SHA-512 系派生與 MAC（HKDF-SHA512、HMAC-SHA512→32B）**，替代傳統 ECC/RSA 與 SHA-256/HMAC-SHA256 組合。核心能力聚焦在**服務接入層、資料存取層**及下文工程化配套元件：
...
| 🔐 高強度對稱棧 | JWT：**HKDF-SHA512→32B**；報文 **HMAC-SHA512→32B** + AES-256-GCM | SHA-512 系派生/MAC，256 位元金鑰面向 Grover 更高邊際 |
| 🔒 防重放攻擊 | 協議 `n`（32B 隨機）+ 時間戳 + `s`（HMAC-SHA512→32B） | 業務/推送統一 MAC（見安全文件） |
...
| **Token 簽發與校驗** | JWT 第三段 | **HKDF-SHA512 → 32B**（非 RFC7519 HS256-HMAC） | 基於 SHA-512 的 KDF 派生，避免經典短雜湊 HMAC 路徑 |
| **會話 Secret** | `GetTokenSecret` | **HKDF-SHA512 → 32B** | 與 Token 綁定的高強度派生金鑰，供後續 MAC/加密 |
| **業務完整性** | 欄位 `s` | **HMAC-SHA512 → 32B** | 統一業務與推送 MAC；256 位元截斷對齊 AES-256 金鑰材料 |
...
- **對稱面（Token 之後）**：Token、Secret、報文 MAC 與加密均落在 **SHA-512 / 256 位元金鑰**體系，相對 SHA-256/HMAC-SHA256 在量子攻擊模型下通常具有更高安全邊際（仍需配合金鑰輪換、短 Token 壽命與 TLS）。
- **邊界說明**：Plan0/1 登入態業務幀不重複攜帶 ML-DSA 外層簽，依賴 **JWT + HMAC-SHA512→32B +（可選）GCM**；若要求**每一幀**均抗量子非對稱簽名，應使用 Plan2 或 RPCX 的 ML-DSA 路徑。傳輸層 **TLS 1.3** 仍建議由閘道/反向代理終止。
...
│  └────────────┴────────────┴────────────┴────────────┴──────┘   │
├─────────────────────────────────────────────────────────────────┤
│           Security & Crypto（抗量子攻擊 + SHA-512 系）              │
│  ┌────────────┬────────────┬────────────┬────────────┬──────┐   │
...
【效能指標摘錄】（2026-05-18，`http_test.go` 本機 1m 壓測；詳見 [`http_benchmark_report.md`](./http_benchmark_report.md)）
• `PostByPlan2`（ML-KEM+ML-DSA Plan2 登入，`/login`）: ≈3,393 TPS  • `PostByPlan01`（JWT+HMAC-SHA512→32B 登入態，`/getUser`）: ≈52,320 TPS
• Plan2 ns/op: 912,683  • Plan01 ns/op: 201,899  • 失敗率: 0.00%
...
- **JWT Token**: 第三段為 **HKDF-SHA512→32B** 校驗域（Header.alg 常為 `HS256` 僅為配置欄位名），支援短 `exp` 與 RBAC
- **會話 Secret**: `GetTokenSecret` 同源 **HKDF-SHA512→32B**，不落庫、按需派生
- **HMAC-SHA512→32B**: 業務 `s` 與推送統一完整性 MAC（`SignBodyMessage`）
- **AES-256-GCM**: 載荷認證加密（按 Plan / 路由啟用）

### 多重認證體系（登入後）

- 登入態 **Plan0/1**：JWT + HMAC-SHA512→32B +（可選）AES-GCM，對稱層採用 256 位元金鑰材料
...
| **雙層校驗（Plan2）** | 身份 + 完整性 | 先驗 **ML-DSA** 外層 `e`，再驗 **HMAC-SHA512→32B** 與 GCM |
...
- **`s`**：對稱 MAC（HMAC-SHA512→32B）；Plan2 另有非對稱 **`e`**（ML-DSA）。
- **Token / Secret**：JWT 第三段與會話金鑰均為 **HKDF-SHA512→32B**，不是 RFC7519 的 HS256-HMAC。
...
| 🔒 強安全 / 支付場景 | Plan2 抗量子登入 + SHA-512 系 Token/MAC（見安全文件） | 金融、支付系統 |
