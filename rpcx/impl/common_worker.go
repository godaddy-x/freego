...
func GetSharedKey(c cache.Cache, cipher fgocrypto.Cipher) ([]byte, error) {
	...
	if len(sharedKey) == 0 { // 如果没有找到缓存则重新处理协商
		...
		res, err := utils.AesGCMEncryptBaseByteResult(sharedKey, utils.HMAC_SHA256_BASE(utils.SHA256_BASE(prkBs), utils.Str2Bytes(utils.GetLocalDynamicSecretKey())), pubBs)
		if err != nil {
		...
		res, err := utils.AesGCMDecryptBaseByteResult(sharedKey, utils.HMAC_SHA256_BASE(utils.SHA256_BASE(prkBs), utils.Str2Bytes(utils.GetLocalDynamicSecretKey())), pubBs)
		if err != nil {
