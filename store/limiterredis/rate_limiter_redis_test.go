...
import (
	...
	lim "github.com/godaddy-x/freego/infra/limiter/contract"
...
func TestRedisRateLimiterOperations(t *testing.T) {
	initRedis()
	// 跳过Redis不可用的环境
	limiter, err := NewRedisRateLimiter(lim.Option{
		Limit:       10.0, // 每秒10个令牌
		Bucket:      5,    // 桶容量5
		Expire:      3000, // 3秒过期
		Distributed: true,
...
func TestRedisRateLimiterConfiguration(t *testing.T) {
	...
	invalidConfigs := []struct {
		...
		{"non-distributed", lim.Option{Limit: 10, Bucket: 5, Distributed: false}},
		{"zero bucket", lim.Option{Limit: 10, Bucket: 0, Distributed: true}},
		{"negative bucket", lim.Option{Limit: 10, Bucket: -1, Distributed: true}},
		{"zero limit", lim.Option{Limit: 0, Bucket: 5, Distributed: true}},
		{"negative limit", lim.Option{Limit: -1, Bucket: 5, Distributed: true}},
	}

	for _, tc := range invalidConfigs {
		t.Run("Invalid_"+tc.name, func(t *testing.T) {
			_, err := NewRedisRateLimiter(tc.opt)
			if err == nil {
