...
type RedisRateLimiter struct {
	option limiter.Option           // 限流器配置
	client *cacheredis.RedisManager // Redis客户端
...
func NewRedisRateLimiter(option limiter.Option) (*RedisRateLimiter, error) {
	// 1. 校验分布式开关
	...
	// 5. 初始化Redis客户端
	client, err := cacheredis.NewRedis()
	if err != nil {
