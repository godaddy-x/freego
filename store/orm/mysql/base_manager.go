...
func hashOptions(opt Option) string {
	return utils.AddStr(opt.DsName, ":", opt.Database, ":", utils.AnyToStr(opt.Timeout),
		":", utils.AnyToStr(opt.OpenTx), ":", utils.AnyToStr(opt.AutoID))
}
		...
		for _, vv := range obv.FieldElem {
			...
			if vv.Primary {
				...
				if vv.FieldKind == reflect.Int64 {
					lastInsertId := reflectx.GetInt64(reflectx.GetPtr(v, obv.PkOffset))
					if lastInsertId == 0 {
						if obv.AutoId {
							continue
						}
						lastInsertId = utils.NextIID()
						reflectx.SetInt64(reflectx.GetPtr(v, vv.FieldOffset), lastInsertId)
					}
					parameter = append(parameter, lastInsertId)
				} else if vv.FieldKind == reflect.String {
					lastInsertId := reflectx.GetString(reflectx.GetPtr(v, obv.PkOffset))
					if len(lastInsertId) == 0 {
						if obv.AutoId {
							continue
						}
						lastInsertId = utils.NextSID()
						reflectx.SetString(reflectx.GetPtr(v, vv.FieldOffset), lastInsertId)
					}
					parameter = append(parameter, lastInsertId)
				} else {
...
func (self *RDBManager) UpdateWithContext(ctx context.Context, data ...sqlc.Object) error {
	...
	if obv.PkKind == reflect.Int64 {
		lastInsertId = reflectx.GetInt64(reflectx.GetPtr(oneData, obv.PkOffset))
		if lastInsertId == 0 {
			return self.Error("[Mysql.Update] data object id is nil")
		}
	} else if obv.PkKind == reflect.String {
		lastInsertId = reflectx.GetString(reflectx.GetPtr(oneData, obv.PkOffset))
		if lastInsertId == "" {
			return self.Error("[Mysql.Update] data object id is nil")
		}
	} else {
...
func (self *RDBManager) BuildPaginationWithContext(ctx context.Context, cnd *sqlc.Cnd, sqlBytes []byte, values []interface{}) ([]byte, error) {
	...
	mysqlDialect := sqlc.MysqlDialect{Dialect: pagination}
