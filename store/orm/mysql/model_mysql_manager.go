...
import (
	...
	reflectx "github.com/godaddy-x/freego/core/reflect"
...
func GetValue(obj interface{}, elem *FieldElem) (interface{}, error) {
	ptr := reflectx.GetPtr(obj, elem.FieldOffset)
	switch elem.FieldKind {
	case reflect.String:
		return reflectx.GetString(ptr), nil
	case reflect.Int:
		ret := reflectx.GetInt(ptr)
		if elem.IsDate {
			if ret == 0 {
				return nil, nil
			}
		...
		return ret, nil
	case reflect.Int8:
		return reflectx.GetInt8(ptr), nil
	case reflect.Int16:
		return reflectx.GetInt16(ptr), nil
	case reflect.Int32:
		ret := reflectx.GetInt32(ptr)
		if elem.IsDate {
			if ret == 0 {
				return nil, nil
			}
	...
	case reflect.Float32:
		return reflectx.GetFloat32(ptr), nil
	case reflect.Float64:
		return reflectx.GetFloat64(ptr), nil
	case reflect.Bool:
		return reflectx.GetBool(ptr), nil
	case reflect.Uint:
		return reflectx.GetUint(ptr), nil
	case reflect.Uint8:
		return reflectx.GetUint8(ptr), nil
	case reflect.Uint16:
		return reflectx.GetUint16(ptr), nil
	case reflect.Uint32:
		return reflectx.GetUint32(ptr), nil
	case reflect.Uint64:
		return reflectx.GetUint64(ptr), nil
	case reflect.Slice:
		switch elem.FieldType {
		case "[]string":
			return getValueJsonStr(reflectx.GetStringArr(ptr))
		case "[]int":
			return getValueJsonStr(reflectx.GetIntArr(ptr))
		case "[]int8":
			return getValueJsonStr(reflectx.GetInt8Arr(ptr))
		case "[]int16":
			return getValueJsonStr(reflectx.GetInt16Arr(ptr))
		case "[]int32":
			return getValueJsonStr(reflectx.GetInt32Arr(ptr))
		case "[]int64":
			return getValueJsonStr(reflectx.GetInt64Arr(ptr))
		case "[]float32":
			return getValueJsonStr(reflectx.GetFloat32Arr(ptr))
		case "[]float64":
			return getValueJsonStr(reflectx.GetFloat64Arr(ptr))
		case "[]bool":
			return getValueJsonStr(reflectx.GetBoolArr(ptr))
		case "[]uint":
			return getValueJsonStr(reflectx.GetUintArr(ptr))
		case "[]uint8":
			return reflectx.GetUint8Arr(ptr), nil
		case "[]uint16":
			return getValueJsonStr(reflectx.GetUint16Arr(ptr))
		case "[]uint32":
			return getValueJsonStr(reflectx.GetUint32Arr(ptr))
		case "[]uint64":
			return getValueJsonStr(reflectx.GetUint64Arr(ptr))
		}
	case reflect.Map:
	...
	case reflect.Ptr:
		switch elem.FieldType {
		case "*string":
			return reflectx.GetStringP(ptr), nil
		case "*int":
			return reflectx.GetIntP(ptr), nil
		case "*int8":
			return reflectx.GetInt8P(ptr), nil
		case "*int16":
			return reflectx.GetInt16P(ptr), nil
		case "*int32":
			return reflectx.GetInt32P(ptr), nil
		case "*int64":
			return reflectx.GetInt64P(ptr), nil
		case "*float32":
			return reflectx.GetFloat32P(ptr), nil
		case "*float64":
			return reflectx.GetFloat64P(ptr), nil
		case "*bool":
			if boolPtr := reflectx.GetBoolP(ptr); boolPtr != nil {
				if *boolPtr {
					return "true", nil
				}
				return "false", nil
			...
			return nil, nil
		case "*uint":
			return reflectx.GetUintP(ptr), nil
		case "*uint8":
			return reflectx.GetUint8P(ptr), nil
		case "*uint16":
			return reflectx.GetUint16P(ptr), nil
		case "*uint32":
			return reflectx.GetUint32P(ptr), nil
		case "*uint64":
			return reflectx.GetUint64P(ptr), nil
		case "*time.Time":
			// 处理 *time.Time 类型的字段
			if timePtr := reflectx.GetTimeP(ptr); timePtr != nil {
				if timePtr.IsZero() {
					return nil, nil
				}
...
func safeBytesToString(b []byte) string {
	//if len(b) == 0 {
	//	// 空数据直接转换，无需创建副本
	//	ret, _ := reflectx.NewString(b)
	//	return ret
	//} else {
	//	// 非空数据创建副本，避免对象池释放问题
	//	v := make([]byte, len(b))
	//	copy(v, b)
	//	ret, _ := reflectx.NewString(v)
	//	return ret
	//}
	//ret, _ := reflectx.NewString(b)
...
func SetValueDefault(obj interface{}, elem *FieldElem, b []byte, needSafe bool) error {
	...
	case reflect.String:
		if needSafe {
			reflectx.SetString(ptr, string(b))
		} else {
			reflectx.SetString(ptr, safeBytesToString(b))
		}
		return nil
	case reflect.Int:
		if elem.IsDate {
		...
		if ret, err := reflectx.NewInt(b); err != nil {
			return err
		} else {
			reflectx.SetInt(ptr, ret)
		}
		return nil
	case reflect.Int8:
		if ret, err := reflectx.NewInt8(b); err != nil {
			return err
		} else {
			reflectx.SetInt8(ptr, ret)
		}
		return nil
	case reflect.Int16:
		if ret, err := reflectx.NewInt16(b); err != nil {
			return err
		} else {
			reflectx.SetInt16(ptr, ret)
		}
		return nil
	case reflect.Int32:
		if elem.IsDate {
			...
			if len(ret) > 0 {
				if rdate, err := utils.Str2FormatTime(ret, modelTime.fmt, modelTime.local); err != nil {
					...
					reflectx.SetInt32(ptr, int32(rdate))
				}
			}
			return nil
		} else if elem.IsDate2 {
		...
		if ret, err := reflectx.NewInt32(b); err != nil {
			return err
		} else {
			reflectx.SetInt32(ptr, ret)
		}
		return nil
	case reflect.Int64:
		if elem.IsDate {
		...
		if ret, err := reflectx.NewInt64(b); err != nil {
			return err
		} else {
			reflectx.SetInt64(ptr, ret)
		}
		return nil
	case reflect.Float32:
		if ret, err := reflectx.NewFloat32(b); err != nil {
			return err
		} else {
			reflectx.SetFloat32(ptr, ret)
		}
		return nil
	case reflect.Float64:
		if ret, err := reflectx.NewFloat64(b); err != nil {
			return err
		} else {
			reflectx.SetFloat64(ptr, ret)
		}
		return nil
	case reflect.Bool:
		str := safeBytesToString(b)
		if str == "true" {
			reflectx.SetBool(ptr, true)
		} else {
			reflectx.SetBool(ptr, false)
		}
		return nil
	case reflect.Uint:
		if ret, err := reflectx.NewUint64(b); err != nil {
			return err
		} else {
			reflectx.SetUint64(ptr, ret)
		}
		return nil
	case reflect.Uint8:
		if ret, err := reflectx.NewUint16(b); err != nil {
			return err
		} else {
			reflectx.SetUint16(ptr, ret)
		}
		return nil
	case reflect.Uint16:
		if ret, err := reflectx.NewUint16(b); err != nil {
		...
		return nil
	case reflect.Uint32:
		if ret, err := reflectx.NewUint32(b); err != nil {
			return err
		} else {
			reflectx.SetUint32(ptr, ret)
		}
		return nil
	case reflect.Uint64:
		if ret, err := reflectx.NewUint64(b); err != nil {
	...
	case reflect.Ptr:
		...
		case "*string":
			...
			reflectx.SetStringP(ptr, &ret)
			return nil
		case "*int":
			if ret, err := reflectx.NewInt(b); err != nil {
				return err
			} else {
				reflectx.SetIntP(ptr, &ret)
			}
			return nil
		case "*int8":
			if ret, err := reflectx.NewInt8(b); err != nil {
		...
		case "*bool":
			...
			if str == "true" {
				boolValue := true
				reflectx.SetBoolP(ptr, &boolValue)
			} else {
				boolValue := false
				reflectx.SetBoolP(ptr, &boolValue)
			}
			return nil
		case "*uint":
			if ret, err := reflectx.NewUint(b); err != nil {
				return err
			} else {
				reflectx.SetUintP(ptr, &ret)
			}
			return nil
		case "*uint8":
			if ret, err := reflectx.NewUint8(b); err != nil {
				return err
			} else {
				reflectx.SetUint8P(ptr, &ret)
			}
			return nil
		case "*uint16":
			if ret, err := reflectx.NewUint16(b); err != nil {
