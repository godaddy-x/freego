	...
	for _, v := range data {
		if obv.PkKind == reflect.Int64 {
			...
			if lastInsertId == 0 {
				...
				reflectx.SetInt64(reflectx.GetPtr(v, obv.PkOffset), lastInsertId)
			}
			...
			if len(lastInsertId) == 0 {
				...
				reflectx.SetString(reflectx.GetPtr(v, obv.PkOffset), lastInsertId)
			}
		} else {
...
func (self *MGOManager) UpdateWithContext(ctx context.Context, data ...sqlc.Object) error {
	...
	for _, v := range data {
		...
		if obv.PkKind == reflect.Int64 {
			pk := reflectx.GetInt64(reflectx.GetPtr(v, obv.PkOffset))
			if pk == 0 {
				return self.Error("[Mongo.Update] data object id is nil")
			}
			lastInsertId = pk
		} else if obv.PkKind == reflect.String {
			pk := reflectx.GetString(reflectx.GetPtr(v, obv.PkOffset))
			if len(pk) == 0 {
				return self.Error("[Mongo.Update] data object id is nil")
			}
			lastInsertId = pk
...
func (self *MGOManager) DeleteWithContext(ctx context.Context, data ...sqlc.Object) error {
	...
	for i, v := range data {
		...
		switch obv.PkKind {
		case reflect.Int64:
			delId = reflectx.GetInt64(reflectx.GetPtr(v, obv.PkOffset))
			if delId == 0 {
				return self.Error("[Mongo.Delete] data object id is nil")
			}
		case reflect.String:
			delId = reflectx.GetString(reflectx.GetPtr(v, obv.PkOffset))
