...
import (
	...
	"github.com/godaddy-x/freego/core/concurrent"
...
var (
	gatewayRateLimiter   limiter.RateLimiter                // 网关级限流器：每秒1000请求，桶容量5000，60秒过期
	methodRateLimiter    = map[string]limiter.RateLimiter{} // 方法级限流器：按API路径存储专用限流器
	defaultMethodLimiter limiter.RateLimiter                // 默认方法级限流器：每秒100请求，桶容量200，30秒过期
	userRateLimiter      limiter.RateLimiter                // 用户级限流器：每用户每秒10请求，桶容量20，30秒过期
)
...
func initRateLimiters() {
	...
	// 初始化网关级限流器
	gatewayRateLimiter = rate.NewRateLimiter(limiter.Option{
	...
	// 初始化默认方法级限流器
	defaultMethodLimiter = rate.NewRateLimiter(limiter.Option{
	...
	// 初始化用户级限流器
	userRateLimiter = rate.NewRateLimiter(limiter.Option{
	...
	methodRateLimiter["/key"] = rate.NewRateLimiter(limiter.Option{
		Limit: 2500, Bucket: 7500, Expire: 30000, Distributed: true,
	})
	methodRateLimiter["/login"] = rate.NewRateLimiter(limiter.Option{
...
func (self *GatewayRateLimiterFilter) DoFilter(chain Filter, ctx *Context, args ...interface{}) error {
	...
	var methodLimiter limiter.RateLimiter
...
func (self *SessionFilter) DoFilter(chain Filter, ctx *Context, args ...interface{}) error {
	...
	if ctx.RouterConfig.UsePlan2 || ctx.RouterConfig.Guest { // 登录接口和游客模式跳过会话认证
		return chain.DoFilter(chain, ctx, args...)
	}
