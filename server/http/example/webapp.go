...
import (
	...
	limiter "github.com/godaddy-x/freego/infra/limiter/contract"
	"github.com/godaddy-x/freego/infra/zlog"
	"github.com/godaddy-x/freego/server/geetest"
...
func (self *MyWebNode) test(ctx *httpsvr.Context) error {
	req := &GetUserReq{}
...
func (self *MyWebNode) getUser(ctx *httpsvr.Context) error {
	req := &GetUserReq{}
	if err := ctx.Parser(req); err != nil {
		return err
	}
...
func (self *MyWebNode) login(ctx *httpsvr.Context) error {
	...
	bs, err := utils.JsonMarshal(&wire.AuthToken{
		Token:   token,
...
func (self *MyWebNode) publicKey(ctx *httpsvr.Context) error {
...
func (self *MyWebNode) testGuestPost(ctx *httpsvr.Context) error {
	fmt.Println(string(ctx.JsonBody.RawData()))
	return self.Json(ctx, map[string]string{"res": "中文测试下Guest响应"})
}

func (self *MyWebNode) testHAX(ctx *httpsvr.Context) error {
	fmt.Println(string(ctx.JsonBody.RawData()))
...
func (self *NewPostFilter) DoFilter(chain httpsvr.Filter, ctx *httpsvr.Context, args ...interface{}) error {
	...
	ctx.AddStorage("httpLog", httpsvr.HttpLog{Method: ctx.Path, LogNo: utils.NextSID(), CreateAt: utils.UnixMilli()})
...
func roleRealm(ctx *httpsvr.Context) (*httpsvr.Permission, error) {
	permission := &httpsvr.Permission{}
...
func NewHTTP() *MyWebNode {
	...
	my.AddErrorHandle(func(ctx *httpsvr.Context, throw ex.Throw) error {
		fmt.Println(throw)
		return throw
	})
	my.AddFilter(&httpsvr.FilterObject{Name: "TestFilter", Order: 100, Filter: &TestFilter{}, MatchPattern: []string{"/getUser"}})
	my.AddFilter(&httpsvr.FilterObject{Name: "NewPostFilter", Order: 100, Filter: &NewPostFilter{}})
	my.AddFilter(&httpsvr.FilterObject{Name: "GeetestFilter", Order: 101, MatchPattern: []string{"/TestGeetest"}, Filter: &GeetestFilter{}})
	return my
}
...
func StartHttpNode() {
	...
	my := NewHTTP()
	my.SetLengthCheck(httpsvr.MAX_BODY_LEN*5, 0, 0)
	my.POST("/test1", my.test, nil)
	my.POST("/getUser", my.getUser, &httpsvr.RouterConfig{AesRequest: false, AesResponse: false})
	my.POST("/testGuestPost", my.testGuestPost, &httpsvr.RouterConfig{Guest: true})
	my.POST("/key", my.publicKey, &httpsvr.RouterConfig{Guest: true})
	my.POST("/login", my.login, &httpsvr.RouterConfig{UsePlan2: true})

	my.POST("/geetest/register", my.FirstRegister, &httpsvr.RouterConfig{UsePlan2: true})
	my.POST("/geetest/validate", my.SecondValidate, &httpsvr.RouterConfig{UsePlan2: true})
...
func configureRateLimiters(my *MyWebNode) {
	...
	my.SetGatewayRateLimiter(limiter.Option{
	...
	my.SetDefaultMethodRateLimiter(limiter.Option{
	...
	// 4. 为敏感接口设置专用限流器配置
	// 登录接口：限制更严格，防止暴力破解
	my.SetMethodRateLimiterByPath("/login", limiter.Option{
	...
	// 用户信息接口：中等限制
	my.SetMethodRateLimiterByPath("/getUser", limiter.Option{
		Limit:       30,    // 每秒30个请求
		Bucket:      60,    // 桶容量60
		Expire:      30000, // 30秒过期时间
		Distributed: true,
	...
	// 公开接口：相对宽松
	my.SetMethodRateLimiterByPath("/key", limiter.Option{
	...
	my.SetUserRateLimiter(limiter.Option{
		Limit:       5,     // 每个用户每秒5个请求
	...
	// 6. 动态调整示例（可在运行时调用）
	// 业务高峰期：提高网关限流阈值
	// my.SetGatewayRateLimiter(limiter.Option{Limit: 800, Bucket: 4000, Expire: 60000, Distributed: true})

	// 活动期间：降低用户级限制
	// my.SetUserRateLimiter(limiter.Option{Limit: 10, Bucket: 20, Expire: 30000, Distributed: true})

	// 维护期间：严格限制所有接口
	// my.SetGatewayRateLimiter(limiter.Option{Limit: 10, Bucket: 50, Expire: 60000, Distributed: true})
