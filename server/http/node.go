...
func (self *Context) GetHmac256Sign(d, n string, t, p, u int64, key []byte) []byte {
	return fwsign.SignBodyMessage(self.Path, d, n, t, p, u, key)
}
...
func (self *Context) Parser(dst interface{}) error {
	...
	context := dto.Context{
		Identify: identify,
		Path:     self.Path,
		System:   &dto.System{Name: self.System.Name, Version: self.System.Version},
	}
...
var (
	keyLimiter = rate.NewRateLimiter(limiter.Option{
...
func (self *Context) CreatePublicKey() (*wire.PublicKey, error) {
	...
	// 增加限流器控制USR访问量
	if !keyLimiter.Allow(utils.AddStr("Limiter:CreatePublicKey:", checkObject.Usr)) {
	...
	if err = fwsign.CheckPublicKey(c, checkObject, cipher); err != nil {
		return nil, err
	}
	...
	requestObject, err := fwsign.CreatePublicKey(serverEkB64, utils.Base64Encode(utils.GetRandomSecure(32)), checkObject.Usr, cipher)
	if err != nil {
		return nil, err
	}

	if err := c.Put(utils.FNV1a64(utils.AddStr(requestObject.Key, ":", requestObject.Usr)), &wire.PrivateKey{Key: dkB64, Noc: requestObject.Noc}, 180); err != nil {
