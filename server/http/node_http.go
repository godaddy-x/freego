...
var (
	defaultCacheObject = cachelocal.NewDefaultLocalCache()
...
func defaultRenderError(ctx *Context, err error) error {
	...
	defer pool.PutJsonResp(resp) // 确保对象池正确释放
	resp.Code = out.Code
	...
	case APPLICATION_JSON:
		...
		if routerConfig.UsePlan2 { // 非登录状态响应
			if ctx.JsonBody.Plan == 2 {
				...
				resp.Data, err = utils.AesGCMEncryptBase(data, key[0:32], fwsign.AppendBodyMessage(ctx.Path, "", resp.Nonce, resp.Time, resp.Plan, ctx.JsonBody.User))
				if err != nil {
					return ex.Throw{Code: http.StatusInternalServerError, Msg: "encryption response data failed", Err: err}
				}
				ctx.DelStorage(SharedKey)
				sign, digest = fwsign.SignAndDigestBodyMessage(ctx.Path, resp.Data, resp.Nonce, resp.Time, resp.Plan, ctx.JsonBody.User, key)
		...
		if wire.PlanRequiresOuterSignature(ctx.JsonBody.Plan) {
