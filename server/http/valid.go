...
func (self *Context) validJsonBodyPlan01Flow(body *wire.JsonBody) error {
	...
	sign, _ := fwsign.SignAndDigestBodyMessage(self.Path, body.Data, body.Nonce, body.Time, body.Plan, body.User, sharedKey)
	...
	if body.Plan == 0 { // 登录状态 P0 Base64
		...
		rawData, err = utils.AesGCMDecryptBase(body.Data, sharedKey[:32], fwsign.AppendBodyMessage(self.Path, "", body.Nonce, body.Time, body.Plan, body.User))
		if err != nil {
...
func (self *Context) validJsonBodyPlan2Flow(body *wire.JsonBody) error {
	// Plan2 独立流程：
	...
	cipher, err = self.CheckOuterSign(cipher, fwsign.DigestBodyMessage(self.Path, body.Data, body.Nonce, body.Time, body.Plan, body.User), utils.Base64Decode(body.Valid))
	if err != nil {
		return err
	}
	...
	sign := fwsign.SignBodyMessage(self.Path, body.Data, body.Nonce, body.Time, body.Plan, body.User, sharedKey)
	if !utils.CompareBase64Sign(sign, body.Sign) {
		return ex.Throw{Code: http.StatusBadRequest, Msg: "request signature invalid"}
	}
...
func (self *Context) validJsonBodyCommon(body *wire.JsonBody) error {
...
func (self *Context) validJsonBodyTimeWindow(body *wire.JsonBody) error {
...
func (self *Context) getPlan01DerivedKey(body *wire.JsonBody) ([]byte, error) {
...
func (self *Context) negotiatePlan2SharedKey(body *wire.JsonBody) ([]byte, error) {
	...
	if err := fwsign.CheckPublicKey(c, public, cipher); err != nil {
		return nil, err
	}

	cacheKey := utils.FNV1a64(utils.AddStr(public.Key, ":", public.Usr))
	var prkObject *wire.PrivateKey
