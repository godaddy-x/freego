...
import (
	...
	"github.com/godaddy-x/freego/server/geetest/sdk"
	httpsvr "github.com/godaddy-x/freego/server/http"
)

var config = Config{}
var geetestCache cache.Cache
...
func getCache() (cache.Cache, error) {
	if geetestCache == nil {
		return nil, utils.Error("geetest cache not configured")
	}
	return geetestCache, nil
}
...
func FirstRegister(ctx *httpsvr.Context) (sdk.GeetestLibResultData, error) {
...
func SecondValidate(ctx *httpsvr.Context) (map[string]string, error) {
	filterObject, err := GetFilterObject(ctx)
	if err != nil {
...
func GetFilterObject(ctx *httpsvr.Context) (string, error) {
