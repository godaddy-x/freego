...
import (
	...
	rate "github.com/godaddy-x/freego/infra/limiterlocal"
...
// 对象池约束：*wire.JsonResp 由 replyData Put 回池；*wire.JsonBody 在 WebSocket 路径为每条消息的栈上/池对象（Process 的 jb），不得异步持有。
...
// 单帧协议体使用 Process(..., jb) 的池化 *wire.JsonBody，避免与 ParallelEnabled 下的并发收包共享指针。
...
func (cc *ConnectionContext) Parser(body []byte, dst interface{}) error {
	...
	src := reflectx.GetPtr(dst, 0)
	req := dto.GetBasePtrReq(src)
	base := dto.BaseReq{
		Context: context,
		Offset:  req.Offset,
		Limit:   req.Limit,
	...
	*((*dto.BaseReq)(unsafe.Pointer(src))) = base
	return nil
}
...
type WsServer struct {
	...
	// ECC和缓存配置（用于Plan 2）
	cipherHook      fwsign.CipherHook                       // Plan2：按 usr 动态加载 ML-DSA Cipher
...
func (eh *ErrorHandler) handleConnectionError(connCtx *ConnectionContext, err error, operation string, fallbackNonce string) {
	...
	if conn := connCtx.WsConn; conn != nil {
		resp := pool.GetJsonResp()
		defer pool.PutJsonResp(resp)
...
func (cm *ConnectionManager) SendToSubject(subject, router string, data interface{}) error {
	...
	// 构造JsonResp格式的推送消息
	jsonResp := pool.GetJsonResp()
	defer pool.PutJsonResp(jsonResp)
	...
	signData := fwsign.SignBodyMessage(router, jsonResp.Data, pushNonce, jsonResp.Time, jsonResp.Plan, 0, utils.Str2Bytes(broadcastKey))
...
func (cm *ConnectionManager) sendToSubjectByJsonResp(subject string, res *wire.JsonResp) error {
	if res.Router == "" {
...
func (mh *MessageHandler) validWebSocketBody(connCtx *ConnectionContext, rawFrame []byte, body *wire.JsonBody) (crypto.Cipher, []byte, error) {
...
func (mh *MessageHandler) resolveRouterConfig(connCtx *ConnectionContext, body *wire.JsonBody) *RouterConfig {
	if connCtx == nil {
		return nil
	}
...
func (mh *MessageHandler) validWebSocketBodyCommon(connCtx *ConnectionContext, body *wire.JsonBody, effectiveConfig *RouterConfig) error {
	if body == nil {
	...
	if wire.JsonBodyRequiresOuterSignature(body.Plan, false) {
...
func (mh *MessageHandler) validWebSocketBodyPlan01Flow(connCtx *ConnectionContext, body *wire.JsonBody) (crypto.Cipher, []byte, error) {
	...
	// Step 2: HMAC 验证（快速拒绝）
	sign, _ := fwsign.SignAndDigestBodyMessage(body.Router, body.Data, body.Nonce, body.Time, body.Plan, body.User, sharedKey)
...
func (mh *MessageHandler) validWebSocketBodyPlan2Flow(connCtx *ConnectionContext, body *wire.JsonBody) (crypto.Cipher, []byte, error) {
	cipher, err := mh.getPQCipher(body.User)
	if err != nil {
		return nil, nil, err
	}
	outerDigest := fwsign.DigestBodyMessage(body.Router, body.Data, body.Nonce, body.Time, body.Plan, body.User)
	...
	sign := fwsign.SignBodyMessage(body.Router, body.Data, body.Nonce, body.Time, body.Plan, body.User, sharedKey)
	if !utils.CompareBase64Sign(sign, body.Sign) {
		DIC.ClearData(sharedKey)
...
func (mh *MessageHandler) validWebSocketBodyPlan2KeyFlow(connCtx *ConnectionContext, body *wire.JsonBody) (crypto.Cipher, []byte, error) {
	cipher, err := mh.getPQCipher(body.User)
	if err != nil {
		return nil, nil, err
	}
	// /key 首次协商阶段还没有共享密钥，HMAC 使用空 key，身份认证由外层 ML-DSA + PublicKey 校验共同完成。
	sign, digest := fwsign.SignAndDigestBodyMessage(body.Router, body.Data, body.Nonce, body.Time, body.Plan, body.User, []byte{})
	if !utils.CompareBase64Sign(sign, body.Sign) {
		return nil, nil, ex.Throw{Code: http.StatusUnauthorized, Msg: "websocket signature verify invalid"}
	}
...
func (mh *MessageHandler) decryptWebSocketData(connCtx *ConnectionContext, body *wire.JsonBody, sharedKey []byte) ([]byte, error) {
	if body == nil {
	...
	case 1: // AES-GCM加密
		...
		rawData, err := utils.AesGCMDecryptBase(d, secret[:32], fwsign.AppendBodyMessage(body.Router, "", body.Nonce, body.Time, body.Plan, body.User))
		if err != nil {
	...
	case 2: // Plan2 AES-GCM
		...
		rawData, err := utils.AesGCMDecryptBase(d, sharedKey[:32], fwsign.AppendBodyMessage(body.Router, "", body.Nonce, body.Time, body.Plan, body.User))
		if err != nil {
...
func (s *WsServer) invokeWsHeartbeatHooks(ctx context.Context, connCtx *ConnectionContext, jb *wire.JsonBody) {
	if s == nil {
		return
	}
...
func (s *WsServer) validateConnectAuthFromRequest(r *http.Request, path string) (*jwt.Subject, []byte, error) {
	...
	if err := fwsign.CheckPublicKey(c, pub, cipher); err != nil {
		return nil, nil, err
	}
	rawToken := make([]byte, len(authHeader))
	copy(rawToken, authHeader)
...
func replyData(connCtx *ConnectionContext, req requestMeta, cipher crypto.Cipher, reply interface{}) {
	...
	aad := fwsign.AppendBodyMessage(req.Router, "", jsonResp.Nonce, jsonResp.Time, jsonResp.Plan, req.User)
	...
	sign, digest := fwsign.SignAndDigestBodyMessage(req.Router, jsonResp.Data, jsonResp.Nonce, jsonResp.Time, jsonResp.Plan, req.User, secret)
...
func parsePlan2PublicKey(rawAuth []byte) (*wire.PublicKey, error) {
...
func (s *WsServer) BuildPlan2KeyResponse(req *wire.PublicKey) (*wire.PublicKey, error) {
	...
	if err := fwsign.CheckPublicKey(c, req, cipherObj); err != nil {
		return nil, err
	}
	...
	resp, err := fwsign.CreatePublicKey(serverEkB64, utils.Base64Encode(utils.GetRandomSecure(32)), req.Usr, cipherObj)
	if err != nil {
		return nil, err
	}
	if err := c.Put(utils.FNV1a64(utils.AddStr(resp.Key, ":", resp.Usr)), &wire.PrivateKey{Key: dkB64, Noc: resp.Noc}, 180); err != nil {
		return nil, err
	}
	return resp, nil
}
...
func (s *WsServer) negotiatePlan2SharedKey(rawAuth []byte, usr int64) ([]byte, error) {
	...
	sharedKey, err := fwsign.HKDFKey(shared, prkObject.Noc)
	if err != nil {
