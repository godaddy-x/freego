...
> **建议阅读顺序**：先看 **「结果总览」**；**`ns/op` / `B/op` / `allocs/op` 中文含义见 §2.4**；再读 **§1～§2**、**§3～§5**、**§6（复现）**。

## 结果总览（速读）

在本机、独立进程、每项 **60s**、GORM **`PrepareStmt: true`** 的前提下：读路径上 FreeGo 在 **FindOne** 与 **FindList（尤其 1000/2000 条）** 的 **`ns/op`** 更低；写路径上 FreeGo 在 **Save**、**Update** 的 **`ns/op`** 也更低；各场景 **失败率均为 0%**（见分项表）。环境与公平性说明见 **§1～§2**；**`ns/op` / `B/op` / `allocs/op` 释义见 §2.4**。
...
### 2.4 benchmark 指标含义（`go test -benchmem`）

以下为 Go `testing` 在 **`-benchmem`** 下常见三列，**均为「每次循环（1 op）」的平均值**：

| 缩写 | 中文说明 | 读数 |
|------|----------|------|
| **`ns/op`** | 单次操作的平均耗时（**纳秒 / 次**） | **数值越小越快** |
| **`B/op`** | 单次操作的平均**堆分配字节数**（**字节 / 次**） | **数值越小越省内存压力** |
| **`allocs/op`** | 单次操作的平均**堆分配次数**（**次 / 次**） | **数值越小分配越轻** |
...
|------|----------------|------|---------------|
| `ns/op`（单次耗时，纳秒/次） | **11,169** | **16,471** | **≈ 1.47×**（GORM 更慢） |
| `B/op`（堆分配字节/次） | **5,701** | **8,798** | **≈ 1.54×** |
| `allocs/op`（堆分配次数/次） | **88** | **130** | **≈ 1.48×** |
| **60s 总执行次数（N）** | **6,956,574** | **4,163,556** | — |
| **平均每秒执行次数（N/60）** | **≈ 115,943 /s** | **≈ 69,393 /s** | **≈ 0.60×** |
| **失败率** | **0.00%（0/6,956,574）** | **0.00%（0/4,163,556）** | — |
...
**结论（本机、本组 60s）**：FindOne 场景下 FreeGo 快于 GORM，延迟差距约 **47%**；GORM 的分配量约高 **54%**。
...
**`ns/op` / `B/op` / `allocs/op`** 的完整释义见 **§2.4**；下表列名中括号内为与 §3、§5 一致的简述。

| 规模 | FreeGo `ns/op`（单次耗时，纳秒/次） | GORM `ns/op`（单次耗时，纳秒/次） | `ns/op` 比值（GORM÷FreeGo） | FreeGo `B/op`（堆分配字节/次） | GORM `B/op`（堆分配字节/次） | FreeGo `allocs/op`（堆分配次数/次） | GORM `allocs/op`（堆分配次数/次） | FreeGo 60s 总次数（N） | GORM 60s 总次数（N） | FreeGo 平均每秒次数 | GORM 平均每秒次数 | FreeGo 失败率 | GORM 失败率 |
|------|-------------------------------------|-----------------------------------|-----------------------------|-------------------------------|-----------------------------|-------------------------------------|-----------------------------------|------------------------|----------------------|--------------------|------------------|---------------|-------------|
| 100 | **165,937** | **253,354** | **≈1.53×** | 164,130 | 176,786 | 3,573 | 3,908 | **416,199** | **262,132** | **≈ 6,937 /s** | **≈ 4,369 /s** | **0.00%** | **0.00%** |
| 500 | **596,669** | **825,536** | **≈1.38×** | 798,299 | 792,622 | 17,577 | 19,126 | **123,879** | **86,836** | **≈ 2,065 /s** | **≈ 1,447 /s** | **0.00%** | **0.00%** |
| 1000 | **422,738** | **1,472,001** | **≈3.48×** | 1,561,076 | 1,796,209 | 34,981 | 38,076 | **172,712** | **48,658** | **≈ 2,879 /s** | **≈ 811 /s** | **0.00%** | **0.00%** |
| 2000 | **751,271** | **3,189,665** | **≈4.25×** | 3,000,998 | 3,197,950 | 69,856 | 75,990 | **94,597** | **22,664** | **≈ 1,577 /s** | **≈ 378 /s** | **0.00%** | **0.00%** |
...
## 5. Save / Update（独立 60s，2026-04-23）

> 说明：`Update` 基准已强制每次更新变更字段（`alias` + `utime`），避免 `affected rows <= 0` 的模糊 warning 干扰判读。

### 5.1 Save 对比

| 指标 | FreeGo `BenchmarkMysqlSave` | GORM `BenchmarkGormSave` | GORM / FreeGo |
|------|-----------------------------|--------------------------|---------------|
| `ns/op`（单次耗时，纳秒/次） | **301,592** | **368,179** | **≈ 1.22×** |
| `B/op`（堆分配字节/次） | **2,707** | **8,425** | **≈ 3.11×** |
| `allocs/op`（堆分配次数/次） | **31** | **75** | **≈ 2.42×** |
| 60s 总执行次数（N） | **218,028** | **212,752** | — |
...
|------|-------------------------------|----------------------------|---------------|
| `ns/op`（单次耗时，纳秒/次） | **180,680** | **358,455** | **≈ 1.98×** |
| `B/op`（堆分配字节/次） | **2,335** | **4,642** | **≈ 1.99×** |
| `allocs/op`（堆分配次数/次） | **25** | **54** | **≈ 2.16×** |
