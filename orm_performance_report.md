...
### 1.2 脚本验证样例（独立进程，每项仅 2s，仅供格式参考）

| 场景 | FreeGo `ns/op` | GORM `ns/op` | GORM / FreeGo |
|------|----------------|--------------|---------------|
| FindOne | 19,198 | 23,766 | ≈1.24× |
| FindList 100 | 218,626 | 315,328 | ≈1.44× |
| FindList 500 | 718,909 | 1,016,100 | ≈1.41× |
| FindList 1000 | 1,095,707 | 1,778,017 | ≈1.62× |
| FindList 2000 | 1,809,318 | 2,916,638 | ≈1.61× |
...
| 规模 | FreeGo `ns/op` | GORM `ns/op` | GORM / FreeGo | FreeGo `B/op` | GORM `B/op` | FreeGo `allocs/op` | GORM `allocs/op` |
|------|----------------|--------------|---------------|---------------|-------------|--------------------|------------------|
| 100 | **165,937** | **253,354** | **≈1.53×** | 164,130 | 176,786 | 3,573 | 3,908 |
| 500 | **596,669** | **825,536** | **≈1.38×** | 798,299 | 792,622 | 17,577 | 19,126 |
| 1000 | **422,738** | **1,472,001** | **≈3.48×** | 1,561,076 | 1,796,209 | 34,981 | 38,076 |
| 2000 | **751,271** | **3,189,665** | **≈4.25×** | 3,000,998 | 3,197,950 | 69,856 | 75,990 |
...
**结论（本机、本组 60s）**：100/500 条场景下 GORM 慢约 **38%～53%**；1000/2000 条差距扩大到约 **3.5×～4.3×**（随行数增大，ORM 映射与内存分配成本被放大）。
