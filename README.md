...
FreeGo 面向高并发与强安全场景：**Plan2 登录/密钥交换**采用 **NIST 抗量子标准算法（ML-KEM-1024 / ML-DSA-87）**；登录后会话与报文统一 **SHA-512 系派生与 MAC（HKDF-SHA512、HMAC-SHA512→32B）**，替代传统 ECC/RSA 与 SHA-256/HMAC-SHA256 组合。核心能力聚焦在**服务接入层、数据访问层**及下文工程化配套组件：
...
| 🔐 高强度对称栈  | JWT：**HKDF-SHA512→32B**；报文 **HMAC-SHA512→32B** + AES-256-GCM | SHA-512 系派生/MAC，256 位密钥面向 Grover 更高边际 |
| 🔒 防重放攻击    | 协议 `n`（32B 随机）+ 时间戳 + `s`（HMAC-SHA512→32B）            | 业务/推送统一 MAC（见安全文档）                    |
...
| **Token 签发与校验**    | JWT 第三段       | **HKDF-SHA512 → 32B**（非 RFC7519 HS256-HMAC） | 基于 SHA-512 的 KDF 派生，避免经典短哈希 HMAC 路径                                    |
| **会话 Secret**         | `GetTokenSecret` | **HKDF-SHA512 → 32B**                          | 与 Token 绑定的高强度派生密钥，供后续 MAC/加密                                        |
| **业务完整性**          | 字段 `s`         | **HMAC-SHA512 → 32B**                          | 统一业务与推送 MAC；256 位截断对齐 AES-256 密钥材料                                   |
...
- **对称面（Token 之后）**：Token、Secret、报文 MAC 与加密均落在 **SHA-512 / 256 位密钥**体系，相对 SHA-256/HMAC-SHA256 在量子攻击模型下通常具有更高安全边际（仍需配合密钥轮换、短 Token 寿命与 TLS）。
- **边界说明**：Plan0/1 登录态业务帧不重复携带 ML-DSA 外层签，依赖 **JWT + HMAC-SHA512→32B +（可选）GCM**；若要求**每一帧**均抗量子非对称签名，应使用 Plan2 或 RPCX 的 ML-DSA 路径。传输层 **TLS 1.3** 仍建议由网关/反向代理终止。
...
│  └────────────┴────────────┴────────────┴────────────┴──────┘   │
├─────────────────────────────────────────────────────────────────┤
│           Security & Crypto（抗量子攻击 + SHA-512 系）              │
│  ┌────────────┬────────────┬────────────┬────────────┬──────┐   │
...
• `PostByPlan2`（ML-KEM+ML-DSA Plan2 登录，`/login`）: ≈3,393 TPS  • `PostByPlan01`（JWT+HMAC-SHA512→32B 登录态，`/getUser`）: ≈52,320 TPS
...
- **JWT Token**: 第三段为 **HKDF-SHA512→32B** 校验域（Header.alg 常为 `HS256` 仅为配置字段名），支持短 `exp` 与 RBAC
- **会话 Secret**: `GetTokenSecret` 同源 **HKDF-SHA512→32B**，不落库、按需派生
- **HMAC-SHA512→32B**: 业务 `s` 与推送统一完整性 MAC（`SignBodyMessage`）
- **AES-256-GCM**: 载荷认证加密（按 Plan / 路由启用）

### 多重认证体系（登录后）

- 登录态 **Plan0/1**：JWT + HMAC-SHA512→32B +（可选）AES-GCM，对称层采用 256 位密钥材料
...
| **双层校验（Plan2）** | 身份 + 完整性    | 先验 **ML-DSA** 外层 `e`，再验 **HMAC-SHA512→32B** 与 GCM                                 |
...
- **`s`**：对称 MAC（HMAC-SHA512→32B）；Plan2 另有非对称 **`e`**（ML-DSA）。
- **Token / Secret**：JWT 第三段与会话密钥均为 **HKDF-SHA512→32B**，不是 RFC7519 的 HS256-HMAC。
...
| 🔒 强安全 / 支付场景 | Plan2 抗量子登录 + SHA-512 系 Token/MAC（见安全文档） | 金融、支付系统  |
