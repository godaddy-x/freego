...
type SDK struct {
	...
	authToken      wire.AuthToken              // JWT认证令牌
...
func (s *SDK) addOuterSign(path string, jsonBody *wire.JsonBody, plan2KeyBootstrap bool) error {
	...
	outerSign, err := cipher.Sign(fwsign.DigestBodyMessage(path, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User))
	if err != nil {
...
func (s *SDK) GetPublicKey() (*crypto.MLKEM1024Object, *wire.PublicKey, crypto.Cipher, error) {
	...
	s.plan2SharedB64 = ""
	s.plan2KemCtB64 = ""
	public, err := fwsign.CreatePublicKey(utils.Base64Encode(utils.GetRandomSecure(32)), utils.Base64Encode(utils.GetRandomSecure(32)), s.ClientNo, cipher)
	if err != nil {
	...
	if err := fwsign.CheckPublicKey(nil, responseObject, cipher); err != nil {
		return nil, nil, nil, err
	}
...
func (s *SDK) PostByPlan2(path string, requestObj, responseObj interface{}) error {
	...
	sharedKey, err := fwsign.HKDFKey(sharedRaw, public.Noc)
	...
	var jsonData []byte
	if v, b := requestObj.(*wire.AuthToken); b {
	...
	d, err := utils.AesGCMEncryptBase(utils.Str2Bytes(jsonBody.Data), sharedKey, fwsign.AppendBodyMessage(path, "", jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User))
	if err != nil {
		return ex.Throw{Msg: "request data AES encrypt failed"}
	}
	jsonBody.Data = d
	sign, _ := fwsign.SignAndDigestBodyMessage(path, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User, sharedKey)
	...
	key, err := fwsign.CreatePublicKey(public.Key, s.plan2KemCtB64, s.ClientNo, cipher)
	if err != nil {
		return err
	}
	...
	respData := pool.GetJsonResp()
	defer pool.PutJsonResp(respData)
	...
	validSign, _ := fwsign.SignAndDigestBodyMessage(path, respData.Data, respData.Nonce, respData.Time, respData.Plan, jsonBody.User, sharedKey)
	// 清理签名验证数据
	...
	dec, err := utils.AesGCMDecryptBase(respData.Data, sharedKey, fwsign.AppendBodyMessage(path, "", respData.Nonce, respData.Time, respData.Plan, jsonBody.User))
	if err != nil {
	...
	if v, b := responseObj.(*wire.AuthToken); b {
...
func (s *SDK) GetAuth() wire.AuthToken {
	return wire.AuthToken{Token: s.authToken.Token, Secret: s.authToken.Secret, Expired: s.authToken.Expired}
}
...
func (s *SDK) PostByPlan01(path string, requestObj, responseObj interface{}, encrypted bool) error {
	...
	if encrypted {
		jsonBody.Plan = 1
		d, err := utils.AesGCMEncryptBase(utils.Str2Bytes(jsonBody.Data), tokenSecret[:32], fwsign.AppendBodyMessage(path, "", jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User))
		if err != nil {
			return ex.Throw{Msg: "request data AES encrypt failed"}
		}
		jsonBody.Data = d
	...
	sign, _ := fwsign.SignAndDigestBodyMessage(path, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User, tokenSecret)
	jsonBody.Sign = utils.Base64Encode(sign)
	...
	validSign, _ := fwsign.SignAndDigestBodyMessage(path, respData.Data, respData.Nonce, respData.Time, respData.Plan, jsonBody.User, tokenSecret)
	// 清理签名验证数据
	defer DIC.ClearData(validSign)
	isSignOK := utils.CompareBase64Sign(validSign, respData.Sign)
	if !isSignOK {
	...
	if respData.Plan == 0 {
		...
		dec, err = utils.AesGCMDecryptBase(respData.Data, tokenSecret[:32], fwsign.AppendBodyMessage(path, "", respData.Nonce, respData.Time, respData.Plan, jsonBody.User))
		if err != nil {
			return ex.Throw{Msg: "post response data AES decrypt failed"}
		}
		if zlog.IsDebug() {
...
func BuildRequestObject(path string, requestObj interface{}, secret string, encrypted ...bool) ([]byte, error) {
	...
	jsonBody.Sign = utils.Base64Encode(fwsign.SignBodyMessage(path, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User, utils.Str2Bytes(secret)))
	bytesData, err := utils.JsonMarshal(jsonBody)
	if err != nil {
