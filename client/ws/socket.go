...
import (
	...
	"github.com/godaddy-x/freego/protocol/pool"
...
func (s *SDK) gwsHandshakeTimeout() time.Duration {
	sec := s.timeout
	if sec <= 0 {
		sec = 30
	}
...
type MessageHandler interface {
	HandleMessage(message *wire.JsonResp) error
}
...
type SDK struct {
	...
	authToken     atomic.Pointer[wire.AuthToken] // JWT认证令牌（原子快照）
...
func (s *SDK) AuthObject(object interface{}) {
	s.authObject = object
}
...
func (s *SDK) SetRawAuthorization(auth string) {
...
func (s *SDK) SetDomain(domain string) {
	s.domain = normalizeWSHost(domain)
}

// GetDomain 返回当前配置的域名/host（只读）。
func (s *SDK) GetDomain() string {
	return s.domain
}

// GetClientNo 返回当前客户端编号（只读）。
func (s *SDK) GetClientNo() int64 {
	return s.clientNo
}

// GetSSL 返回是否使用 wss（只读）。
func (s *SDK) GetSSL() bool {
	return s.ssl
}

func (s *SDK) getTokenSecretForSigning() string {
...
func (s *SDK) decodeTokenSecretForSend() ([]byte, error) {
...
func (s *SDK) SetClientNo(clientNo int64) {
	s.clientNo = clientNo
}

func (s *SDK) SetSSL(ssl bool) {
	s.ssl = ssl
}
...
func (s *SDK) SetPushMessageCallback(callback func(router string, data []byte)) {
...
func (s *SDK) SetWebSocketPath(path string) error {
...
func (s *SDK) getURI(path string) string {
	var p string
	if s.ssl {
...
func (s *SDK) verifyPushMessageSignature(res *wire.JsonResp) error {
	if res.Router == "" {
	...
	// 使用服务器推送专用签名密钥进行验证
	// 注意：这与服务器端的签名逻辑保持一致
	expectedSign := fwsign.SignBodyMessage(res.Router, res.Data, res.Nonce, res.Time, res.Plan, 0, utils.Str2Bytes(s.broadcastKey))
...
func (s *SDK) decryptPushMessageData(res *wire.JsonResp) ([]byte, error) {
	if res.Data == "" {
...
func (s *SDK) addOuterSign(jsonBody *wire.JsonBody, plan2KeyBootstrap bool) error {
	if !wire.JsonBodyRequiresOuterSignature(jsonBody.Plan, plan2KeyBootstrap) {
		return nil
	}
	...
	outerSign, err := cipher.Sign(fwsign.DigestBodyMessage(jsonBody.Router, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User))
	if err != nil {
...
func (s *SDK) verifyOuterSign(path string, usr int64, respData *wire.JsonResp) error {
	if !wire.PlanRequiresOuterSignature(respData.Plan) {
		return nil
	}
	if s.mldsaObject == nil {
		return ex.Throw{Msg: "ML-DSA object not configured"}
	...
	if err := cipher.Verify(fwsign.DigestBodyMessage(path, respData.Data, respData.Nonce, respData.Time, respData.Plan, usr), outerSignData); err != nil {
...
func (s *SDK) ConnectWebSocket() error {
	s.reconnectMutex.Lock()
...
func (s *SDK) isReconnectEnabled() bool {
	s.reconnectMutex.Lock()
...
// 返回: 连接成功返回nil，否则返回连接失败的错误信息
func (s *SDK) connectWebSocketInternal(path string, isInitial bool) error {
...
func (s *SDK) SendWebSocketRawBody(body *wire.JsonBody, waitResponse bool, timeout int64) (*wire.JsonResp, error) {
	if body == nil {
	...
	var respChan chan *wire.JsonResp
	if waitResponse {
		respChan = make(chan *wire.JsonResp, 1)
...
func (s *SDK) ConnectWebSocketWithRawAuth(path, authHeader string) error {
	if len(authHeader) == 0 {
...
func (s *SDK) buildPlan2BootstrapAuthorization() (string, error) {
	if s.mldsaObject == nil {
		return "", ex.Throw{Msg: "ML-DSA object not configured"}
	}
	cipher, exists := s.mldsaObject[s.clientNo]
	...
	pub, err := fwsign.CreatePublicKey(
...
func (s *SDK) GetWebSocketPlan2Auth(keyRouter string, timeout int64) (string, []byte, error) {
	...
	reqPublic, err := fwsign.CreatePublicKey(
		utils.Base64Encode(utils.GetRandomSecure(32)),
		utils.Base64Encode(utils.GetRandomSecure(32)),
		s.clientNo,
		cipher,
	...
	jsonBody := pool.GetJsonBody()
	defer pool.PutJsonBody(jsonBody)
	...
	sign, _ := fwsign.SignAndDigestBodyMessage(jsonBody.Router, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User, []byte{})
	...
	validSign, _ := fwsign.SignAndDigestBodyMessage(keyRouter, resp.Data, resp.Nonce, resp.Time, resp.Plan, s.clientNo, []byte{})
	...
	if err := fwsign.CheckPublicKey(nil, serverPub, cipher); err != nil {
		return "", nil, err
	}
	...
	sharedKey, err := fwsign.HKDFKey(sharedRaw, serverPub.Noc)
	if err != nil {
		return "", nil, ex.Throw{Msg: "HKDF shared key failed"}
	}

	authPub, err := fwsign.CreatePublicKey(serverPub.Key, kemCtB64, s.clientNo, cipher)
	if err != nil {
...
func (s *SDK) LoginByWebSocketPlan2Auto(keyRouter, loginRouter string, requestObj, responseObj interface{}, timeout int64) error {
...
func (s *SDK) SendWebSocketPlan2Message(router string, requestObj, responseObj interface{}, sharedKey []byte, timeout int64) error {
	...
	enc, err := utils.AesGCMEncryptBase(jsonData, sharedKey[:32], fwsign.AppendBodyMessage(jsonBody.Router, "", jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User))
	if err != nil {
		return ex.Throw{Msg: "plan2 data encrypt failed"}
	}
	jsonBody.Data = enc
	signData, _ := fwsign.SignAndDigestBodyMessage(jsonBody.Router, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User, sharedKey)
	...
	validSign, _ := fwsign.SignAndDigestBodyMessage(router, resp.Data, resp.Nonce, resp.Time, resp.Plan, s.clientNo, sharedKey)
	defer DIC.ClearData(validSign)
	if !utils.CompareBase64Sign(validSign, resp.Sign) {
	...
	switch resp.Plan {
	case 2, 1:
		dec, err = utils.AesGCMDecryptBase(resp.Data, sharedKey[:32], fwsign.AppendBodyMessage(router, "", resp.Nonce, resp.Time, resp.Plan, s.clientNo))
		if err != nil {
...
func (s *SDK) LoginByWebSocketPlan2(loginRouter string, requestObj interface{}, sharedKey []byte, timeout int64) error {
...
func (s *SDK) prepareHeartbeatMessage(router string, data interface{}) ([]byte, string, error) {
	...
	signData, _ := fwsign.SignAndDigestBodyMessage(jsonBody.Router, jsonBody.Data, jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User, tokenSecret)
	jsonBody.Sign = utils.Base64Encode(signData)
...
func (s *SDK) prepareWebSocketMessage(jsonBody *wire.JsonBody, data interface{}) (*wire.JsonBody, []byte, error) {
	// 序列化数据
	jsonData, err := utils.JsonMarshal(data)
	if err != nil {
	...
	// 根据plan决定是否加密
	if jsonBody.Plan == 1 {
		encryptedData, err := utils.AesGCMEncryptBase(jsonData, tokenSecret[:32], fwsign.AppendBodyMessage(jsonBody.Router, "", jsonBody.Nonce, jsonBody.Time, jsonBody.Plan, jsonBody.User))
		if err != nil {
...
func (s *SDK) sendWebSocketAuthHandshake(conn *gws.Conn, path string) error {
	// 使用通用方法准备握手数据
	jsonBody := pool.GetJsonBody()
	defer pool.PutJsonBody(jsonBody)
	jsonBody.Time = utils.UnixSecond()
	...
	case response := <-respChan:
		...
		// 构建签名字符串（使用握手路径）
		validSign, _ := fwsign.SignAndDigestBodyMessage(path, response.Data, response.Nonce, response.Time, response.Plan, jsonBody.User, tokenSecret)
		defer DIC.ClearData(validSign)
		...
		if response.Plan == 1 {
			// AES解密
			decryptedData, err = utils.AesGCMDecryptBase(response.Data, tokenSecret[:32], fwsign.AppendBodyMessage(path, "", response.Nonce, response.Time, response.Plan, jsonBody.User))
			if err != nil {
...
func (s *SDK) SendWebSocketMessage(router string, requestObj, responseObj interface{}, waitResponse, encryptRequest bool, timeout int64) error {
	// 使用通用方法准备消息数据
	jsonBody := pool.GetJsonBody()
	defer pool.PutJsonBody(jsonBody)
	jsonBody.Time = utils.UnixSecond()
	...
	var respChan chan *wire.JsonResp
	if waitResponse {
		respChan = make(chan *wire.JsonResp, 1) // 缓冲1，避免阻塞
...
// response: 响应数据映射
// 返回: 验证后的响应数据和可能的错误信息
func (s *SDK) verifyWebSocketResponseFromJsonResp(path string, result interface{}, jsonResp *wire.JsonResp) error {
	...
	validSign, _ := fwsign.SignAndDigestBodyMessage(path, jsonResp.Data, jsonResp.Nonce, jsonResp.Time, jsonResp.Plan, s.clientNo, tokenSecret)
	defer DIC.ClearData(validSign)
	...
	if wire.PlanRequiresOuterSignature(jsonResp.Plan) {
	...
	if jsonResp.Plan == 1 {
		var decErr error
		decryptedData, decErr = utils.AesGCMDecryptBase(jsonResp.Data, tokenSecret[:32], fwsign.AppendBodyMessage(path, "", jsonResp.Nonce, jsonResp.Time, jsonResp.Plan, s.clientNo))
		if decErr != nil {
...
func (s *SDK) websocketMessageListenerHandle(body []byte) {
	...
	if loaded {
		if respChan, ok := authRespChanVal.(chan *wire.JsonResp); ok {
			select {
	...
	if respChanVal, loaded := s.responseMap.LoadAndDelete(res.Nonce); loaded {
		respChan, ok := respChanVal.(chan *wire.JsonResp)
		if ok {
			select {
			case respChan <- &messageCopy:
				if zlog.IsDebug() {
	...
	if res.Code == 300 {
		...
		if sub, exists := s.subscriptions.Load(res.Router); exists {
			if subscription, ok := sub.(*Subscription); ok && subscription.active && subscription.Handler != nil {
				// 异步调用处理器，避免阻塞消息监听
				go func(handler MessageHandler, msg *wire.JsonResp) {
...
func (s *SDK) handlePushMessage(message map[string]interface{}) {
	// 这里可以添加自定义的推送消息处理逻辑
	// 例如：触发事件、更新UI状态等
	if zlog.IsDebug() {
...
func (s *SDK) DisconnectWebSocket() {
...
func (s *SDK) disconnectWebSocketNoReconnect() {
...
func (s *SDK) disconnectWebSocketInternal(triggerReconnect bool, eventConn *gws.Conn) {
	s.connMutex.Lock()
...
func (s *SDK) calculateReconnectIntervalLocked() time.Duration {
...
func (s *SDK) calculateReconnectInterval() time.Duration {
	s.reconnectMutex.Lock()
	defer s.reconnectMutex.Unlock()
	return s.calculateReconnectIntervalLocked()
}

func (s *SDK) startReconnectProcess() {
...
func (s *SDK) reconnectLogFields(attempt int, path string, backoff time.Duration) []zap.Field {
	fields := []zap.Field{
...
func (s *SDK) getConnectedPathLocked() string {
	if s.connectedPath == "" {
...
func (s *SDK) ForceReconnect() error {
	s.reconnectMutex.Lock()
	s.reconnectAttempts = 0
	s.lastReconnectTime = time.Time{}
...
func (s *SDK) IsWebSocketConnected() bool {
	s.connMutex.Lock()
...
func (s *SDK) triggerTokenExpiredCallbackSync() {
	if s.onTokenExpired == nil {
		return
	}
	if s.valid() || len(s.rawAuthHeader) > 0 {
...
func (s *SDK) SetReconnectConfig(enabled bool, maxAttempts int, initialInterval, maxInterval time.Duration) {
	s.reconnectMutex.Lock()
	defer s.reconnectMutex.Unlock()
...
func (s *SDK) GetReconnectStatus() (enabled bool, attempts int, maxAttempts int, reconnecting bool, nextReconnectTime time.Time) {
	s.reconnectMutex.Lock()
	defer s.reconnectMutex.Unlock()
...
// 返回: 订阅ID和可能的错误信息
func (s *SDK) SubscribeMessage(router string, handler MessageHandler) (subscriptionID string, err error) {
	if handler == nil {
...
func (s *SDK) UnsubscribeMessage(router string) error {
...
func (s *SDK) resubscribeAfterReconnect() {
...
func (s *SDK) GetSubscriptions() map[string]*Subscription {
...
// func (h *ChatMessageHandler) HandleMessage(message *wire.JsonResp) error {
//     fmt.Printf("收到聊天消息: %s\n", message.Data)
//     return nil
// }
//
...
// response, err := wsSdk.SendWebSocketMessage("/ws/chat", request, &wire.JsonResp{}, true, true, 5)
//
// // 5. 持续接收推送消息
