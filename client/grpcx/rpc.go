...
import (
	...
	"github.com/godaddy-x/freego/protocol/sign"
...
type CipherHook = sign.CipherHook

func New(address string) *RPC {
	return &RPC{
		Address:  address,
		SSL:      false,
		timeout:  60,
...
func (r *RPC) post(router string, requestObj, responseObj proto.Message, timeout int64) error {
	...
	s := pb.RPCXRequestDigestS(req.D.Value, req.N, req.T, req.P, req.R, req.U)
	...
	if err := pb.UnpackAny(resp.D, responseObj); err != nil {
...
func (r *RPC) verifyResponse(resp *pb.CommonResponse) error {
	...
	sWant := pb.RPCXResponseDigestS(resp.D.Value, resp.N, resp.T, resp.P, resp.R, r.clientNo, resp.C, resp.M)
