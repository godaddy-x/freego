...
var (
	localLimiterCache = cachelocal.NewLocalCache()
)

func NewRateLimiter(option limiter.Option) limiter.RateLimiter {
	// local-only; redis limiter lives in store/limiterredis
	// 本地模式：验证和修正配置参数
	if option.Bucket <= 0 {
